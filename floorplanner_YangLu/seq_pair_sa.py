...
class SeqPairSA(Annealer) :
  ...
  def __init__(self, state, sp:seq_pair_blockage) :
    self.sp = sp
  ...
  def move_local_rev(self, sigma) :
    i = random.randint(0, self.num_block-1)
    di = max(int(random.gauss(0, sigma)), 1)
    j = max(min(i+di, self.num_block-1), 0)
    if i == j :
      return 
  ...
  def energy(self) :
    ...
    shape =  self.state[2*self.num_block:]
...
if __name__ == '__main__':
  ...
  sp = seq_pair_blockage(blks_info_ami49, do_adapt = True)
